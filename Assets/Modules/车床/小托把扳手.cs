...
public class 小托把扳手 : MonoBehaviour
{
    ...
    public  float nowZposition;     // 物体Z位置
    public  float initialZPosition;  // 物体初始Z位置
    public Camera mainCamera;
    private float initialMouseX;     // 鼠标初始X位置

    private void Start()
    {
        initialZPosition = objectToMove.position.z; // 记录初始Z位置
    }
    ...
    void Update()
    {
        ...
        if (isDragging)
        {
            ...
            nowZposition = objectToMove.position.z;

            // 计算新的X位置
            float deltaZ = nowZposition - initialZPosition;

            // 更新物体位置
            //objectToMove.position = new Vector3(newXPosition, objectToMove.position.y, objectToMove.position.z);

            // 根据鼠标移动方向更新旋转
            if (isX > 0 && nowZposition -initialZPosition <= maxMoveDistance) // 鼠标向左移动
            {
                objectToMove.position -= Vector3.back * moveSpeed * Time.deltaTime;
                objectToRotate.Rotate(Vector3.forward, sensitivity * Time.deltaTime);
            }
            else if (isX < 0 && nowZposition - initialZPosition >= -maxMoveDistance) // 鼠标向右移动
            {
                objectToMove.position += Vector3.back * moveSpeed * Time.deltaTime;
                objectToRotate.Rotate(Vector3.forward, -sensitivity * Time.deltaTime);
            }
